        ...
        <?php else: ?>
            <div class="row">
                <div class="col-12">
                    ...
                    <p class="text-muted small">Diposting oleh <strong><?= htmlspecialchars($article['penulis']) ?></strong>
                        pada <?= date('d F Y', strtotime($article['tanggal_posting'] ?? 'now')) ?></p>
                    <?php if (!empty($article['gambar_utama'])): ?>
                        <div class="mb-4">
                            <img src="/<?= htmlspecialchars($article['gambar_utama']) ?>"
                                alt="<?= htmlspecialchars($article['judul']) ?>" class="img-fluid rounded-3 w-100"
